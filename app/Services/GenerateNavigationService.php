...
class GenerateNavigationService
{
    public static function generate(): void
    {
        ...
        }])->where('navigation_type_id', 1)->orderBy('order_column', 'ASC')->get();
        $admin_navigation = Navigation::with(['children' => function ($query) {
            $query->orderBy('order_column', 'ASC');
        }])->where('navigation_type_id', 2)->orderBy('order_column', 'ASC')->get();
        $usercp_navigation = Navigation::with(['children' => function ($query) {
            $query->orderBy('order_column', 'ASC');
        }])->where('navigation_type_id', 5)->orderBy('order_column', 'ASC')->get();
        try {
